...
4. **Wire the commit-time gates (git-level, non-bypassable).** The agent-time gates (`gate-done`, `format`, `guard-bash`) already come from the plugin's `hooks.json` automatically — no action needed. For the **commit-time** board + docs gates, set up a git hook:
