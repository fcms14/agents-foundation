...
3. **Dispatch implement.** `git mv` each started task `ready → active`. Dispatch the agent named in its `agent:` frontmatter on **Sonnet**, passing the task file as the contract. Parallel subagents in isolated worktrees are the primary speed lever — use them whenever 2+ runnable tasks touch **disjoint areas** (different apps/services). **Pre-assign migration numbers** to parallel backend tasks before dispatch to avoid drizzle journal collisions, and avoid putting two tasks that edit the SAME app/service in one parallel batch (they collide on shared barrels/modules — see [[parallel-worktrees-same-service-integration]]).
