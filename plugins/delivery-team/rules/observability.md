...
- Logging is a **single responsibility** owned by a dedicated class, reused across HTTP (interceptor), workers (BullMQ), and gateways (WebSocket). See `backend.md`.
- Correlation id flows through HTTP → queue → downstream so a single request is traceable end-to-end.
- The logger exposes an **`appendContext(fields)`** API so a unit can attach specific structured fields to the current log scope (e.g. `routeId`, `driverId`, job metadata) **without** scattering `console.log` calls. Context is enriched through the abstraction, never by ad-hoc logging in business code.
...
- PostHog is the product-analytics layer (see `frontend.md`). Treat it as observability for user behavior, kept separate from backend operational logs.
