...
- **Load tests — Artillery.** Every performance-sensitive endpoint (ingestion, feeds, optimization) gets an Artillery scenario asserting latency/throughput targets. Load configs live in the repo and run in CI or on demand.
...
- **Full-stack e2e** (Playwright over the running stack, e.g. register→approve→login) — at **vertical-slice milestones**, a handful of key journeys; not every task.
- **Artillery (load)** — **just-in-time per performance-sensitive endpoint**, written in the task that ships it (ingestion, GPS batch, optimizer). The Artillery **harness/config is scaffolded early** so adding a scenario is cheap; there is **no single final load-test task** and we do **not** wait until the end.
