...
The **technology** half of the foundation: the implementer agents and the conventions for a specific stack. This v0 is an **as-built** baseline captured from a real product (NestJS + React/Turborepo + Drizzle + Railway) — a working starting point, not yet your refined preferences. Domain names in the rules (apps, services, tables) are **illustrative examples**; swap them for your own.
...
This is a **v0 baseline captured from a real product**, kept so future agents maintain consistency with what exists. The opinionated future work:

- **Name and codify the de-facto design pattern** into explicit conventions (folder shape, file naming, import boundaries) — reacting to concrete likes/dislikes rather than inventing.
...
Until then, treat the rules here as the current contract; refine them in your product repo.
